...
export const createRelationshipEdge = (fst, snd) => {
  console.assert([fst.type, snd.type].includes(types.RELATIONSHIP));
	const cardinality = snd.cardinality
  let source = null;
  let target = null;
  if (snd.type === types.ENTITY) {
    source = snd;
    target = fst;
  } else {
    source = fst;
