...
export function GeneralisationAndSubsets({
	...
	const addGeneralisation = () => {
		functions.addElement(types.GENERALISATION, createGeneralisation(parent.id, parent.pos));
	};
...
export function Subsets({ generalisation, children, functions, ctx }) {
	...
	const addSubsetDisplay = (generalisation) => {
		if (ctx.action === actions.SELECT.ADD_SUBSET && selectedGeneralisation === generalisation) {
			return <AddingSubset {...ctx} {...functions} generalisation={generalisation} />;
		} else {
			return (
				<div className="toolbar-text-action" onClick={() => addSubset(generalisation)}>
					+ Add Subset
				</div>
			);
		}
